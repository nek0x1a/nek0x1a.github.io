---
sidebar_position: 3
description: 以 Debian 12 为基础的虚拟服务器配置
---
...
${remotehost}:${remotepath} ${localpath}  nfs    rw,noatime,hard,timeo=150,retrans=3,_netdev 0      0
```
...
:::warning
若 Docker 运行依赖了远程网络文件夹，可能会出现 Docker 启动后 NFS 还没有挂载好的情况，若出现，则需要配置 Docker 在挂载完成之后再运行。
:::
...
```text title="/lib/systemd/system/docker.service" {4,6}
[Unit]
